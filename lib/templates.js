...
AsObject.prototype.emit = function(context, target) {
  ...
  this.object[key] = target;
  this.addListeners(target, key);
};
AsObject.prototype.addListeners = function(target, key) {
...
AsArray.prototype.emit = function(context, target) {
  ...
  for (var i = this.array.length; i--;) {
    ...
    if (mask & 4) {
      this.array.splice(i + 1, 0, target);
      this.addListeners(target);
      return;
    }
  }
  // Add to the beginning if before all items
  this.array.unshift(target);
  this.addListeners(target);
};
AsArray.prototype.addListeners = function(target) {
